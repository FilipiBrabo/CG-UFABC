...
void OpenGLWidget::switchDiagonal(bool changeD)
{

        makeCurrent();
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vboIndices);
        unsigned int * indices =
                (unsigned int *) glMapBufferRange(GL_ELEMENT_ARRAY_BUFFER, 0,

                                                2 * 3 * sizeof (unsigned int), GL_MAP_WRITE_BIT);
        if (changeD) {
            indices [0] = 0;
            indices [1] = 1;
            indices [2] = 3;
...
/* Métodos para fazer a tela piscar nas cores preto e branco
 * void OpenGLWidget::initializeGL() {
 *   initializeOpenGLFunctions();
